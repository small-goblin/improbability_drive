...
function get_vrc {
    curl $1 \
        ...
        -H 'User-Agent: Improbability Drive Mk.1.1' \
...
get_vrc $worlds_api $apiKey $auth | jq ".[${rann}]" > world.json 
...
get_vrc $instance_URL $apiKey $auth
...
payload="@world_payload.json"

post_vrc $payload $WEBHOOK